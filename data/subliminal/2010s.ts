import type { SongList } from "../../timeline/types";

export const subliminal2010s: SongList = [
	{
		name: "תן כוח (Ten Koah / Give Me Strength)",
		artist: "Subliminal (סאבלימינל)",
		collaborators: ["David D'Or (דוד ד'אור)"],
		language: "Hebrew",
		published_date: "2006",
		lyric_sample: {
			hebrew:
				"תן לי כוח להמשיך / תן לי אומץ לא לוותר / במציאות הזאת קשה / אבל אני לא אפסיק להיאבק",
			english_translation:
				"Give me strength to continue / Give me courage not to give up / This reality is hard / But I won't stop fighting",
		},
		links: {
			youtube: "https://www.youtube.com/results?search_query=subliminal+ten+koah",
		},
	},
	{
		name: "קלאסית ופרסי (Klasit Ve Parsi / Classical and Persian)",
		artist: "Subliminal (סאבלימינל)",
		collaborators: ["Miri Ben-Ari (מירי בן אריה)", "Joe Budden"],
		language: "Hebrew/English",
		published_date: "2006",
		lyric_sample: {
			hebrew:
				"על הזהות היהודית-מזרחית, על הערבוב בין תרבויות",
			english_translation:
				"About Jewish-Eastern identity, about mixing cultures",
		},
		links: {
			youtube: "https://www.youtube.com/results?search_query=subliminal+klasit+ve+parsi",
		},
	},
	{
		name: "המקום המושלם (Hamakom Hamushlam / The Perfect Place)",
		artist: "Subliminal (סאבלימינל)",
		language: "Hebrew",
		published_date: "2006",
		lyric_sample: {
			hebrew:
				"על החיפוש אחר מקום בטוח, על הביטחון והשלווה",
			english_translation:
				"About searching for a safe place, about security and peace",
		},
		links: {
			youtube: "https://www.youtube.com/results?search_query=subliminal+hamakom+hamushlam",
		},
	},
	{
		name: "טורו (Toro)",
		artist: "Subliminal (סאבלימינל)",
		language: "Hebrew",
		published_date: "2006",
		lyric_sample: {
			hebrew:
				"שיר בסגנון לטינו-ישראלי, על חיוניות וכוח",
			english_translation:
				"Song in Latin-Israeli style, about vitality and strength",
		},
		links: {
			youtube: "https://www.youtube.com/results?search_query=subliminal+toro",
		},
	},
	{
		name: "לפעמים אני מרגיש (Lifamim Ani Margish / Sometimes I Feel)",
		artist: "Subliminal (סאבלימינל)",
		collaborators: ["David Broza (דוד ברוזה)"],
		language: "Hebrew",
		published_date: "2006",
		lyric_sample: {
			hebrew:
				"על רגשות מעורבים לגבי המצב הביטחוני והפוליטי",
			english_translation:
				"About mixed feelings regarding the security and political situation",
		},
		links: {
			youtube: "https://www.youtube.com/results?search_query=subliminal+lifamim+ani+margish",
		},
	},
	{
		name: "In Tel Aviv",
		artist: "Subliminal (סאבלימינל)",
		collaborators: ["Wyclef Jean"],
		language: "English/Hebrew",
		published_date: "2006",
		lyric_sample: {
			hebrew:
				"שיר בינלאומי על תל אביב, על העיר כמרכז תרבות",
			english_translation:
				"International song about Tel Aviv, about the city as a cultural center",
		},
		links: {
			youtube: "https://www.youtube.com/results?search_query=subliminal+in+tel+aviv",
		},
	},
	{
		name: "ארץ אהובה (Eretz Ahuva / Beloved Land)",
		artist: "Subliminal (סאבלימינל)",
		collaborators: ["Shlomi Shabat (שלומי שבת)"],
		language: "Hebrew",
		published_date: "2006",
		lyric_sample: {
			hebrew:
				"שיר אהבה לארץ ישראל, על הקשר הרוחני לארץ",
			english_translation:
				"Love song to Israel, about spiritual connection to the land",
		},
		links: {
			youtube: "https://www.youtube.com/results?search_query=subliminal+eretz+ahuva",
		},
	},
	{
		name: "לא יכול לישון (Lo Yachol Lishon / Can't Sleep)",
		artist: "Subliminal (סאבלימינל)",
		language: "Hebrew",
		published_date: "2006",
		lyric_sample: {
			hebrew:
				"לא יכול לישון בלילה / כל הזמן חושב מה יהיה / פחדים ודאגות / על העתיד שלנו - על החרדה והפחד מהמצב הביטחוני",
			english_translation:
				"Can't sleep at night / Always thinking what will be / Fears and worries / About our future - about anxiety and fear from security situation",
		},
		links: {
			youtube: "https://www.youtube.com/results?search_query=subliminal+lo+yachol+lishon",
		},
	},
	{
		name: "בדיוק כשחשבתם שהכל נגמר (Bediuk Kshechashavtem Shehacol Nigmar / Just When You Thought It Was Over)",
		artist: "Subliminal (סאבלימינל)",
		language: "Hebrew",
		published_date: "2006",
		lyric_sample: {
			hebrew:
				"שיר הכותרת של האלבום, על חזרה חזקה ותקווה",
			english_translation:
				"Title song of the album, about strong comeback and hope",
		},
		links: {
			youtube: "https://www.youtube.com/results?search_query=subliminal+bediuk",
		},
	},
	{
		name: "אדון עולם עד מתי (Adon Olam Ad Matai / God Almighty, When Will It End?)",
		artist: "Subliminal (סאבלימינל)",
		collaborators: ["Miri Ben-Ari (מירי בן אריה)"],
		language: "Hebrew",
		published_date: "2007",
		lyric_sample: {
			hebrew:
				"דקת דומיה לא תספיק לשישה מליון / אף פעם לא נוכל לקלוט את גודל האסון - שיר היפ-הופ ראשון על השואה",
			english_translation:
				"A minute of silence won't be enough for six million / We can never comprehend the magnitude of the disaster - first hip-hop song about the Holocaust",
		},
		links: {
			youtube: "https://www.youtube.com/watch?v=gXLOlQK2r-I",
		},
	},
	{
		name: "היצירה שלי (My Creation)",
		artist: "Subliminal (סאבלימינל)",
		language: "Hebrew",
		published_date: "2012",
		lyric_sample: {
			hebrew:
				"אלבום אוסף עם גרסאות מחודשות של שירים קודמים, כולל התקווה, הפינאלי, בילאדי",
			english_translation:
				"Collection album with renewed versions of previous songs, including Hope, The Finale, Biladi",
		},
		links: {
			youtube: "https://www.youtube.com/results?search_query=subliminal+my+creation",
		},
	},
	{
		name: "חלום של כל גבר (Chalom Shel Kol Gever / Every Man's Dream)",
		artist: "Subliminal (סאבלימינל)",
		language: "Hebrew",
		published_date: "2015",
		lyric_sample: {
			hebrew:
				"אלבום פופולרי יותר, עם מסרים חיוביים על החיים והחלומות",
			english_translation:
				"More popular album, with positive messages about life and dreams",
		},
		links: {
			youtube: "https://www.youtube.com/results?search_query=subliminal+every+mans+dream",
		},
	},
	{
		name: "צבע לחיים (Tzeva Lechayim / Color to Life)",
		artist: "Subliminal (סאבלימינל)",
		language: "Hebrew",
		published_date: "2016",
		lyric_sample: {
			hebrew:
				"אלבום עם מסרים חיוביים על צבע וחיים",
			english_translation:
				"Album with positive messages about color and life",
		},
		links: {
			youtube: "https://www.youtube.com/results?search_query=subliminal+color+to+life",
		},
	},
	{
		name: "Jew-Niversal",
		artist: "Subliminal (סאבלימינל)",
		language: "English/Hebrew",
		published_date: "2011",
		lyric_sample: {
			hebrew:
				"אלבום בינלאומי עם שירים באנגלית, על הזהות היהודית בעולם",
			english_translation:
				"International album with songs in English, about Jewish identity in the world",
		},
		links: {
			youtube: "https://www.youtube.com/results?search_query=subliminal+jew+niversal",
		},
	},
];
