...
export default async function TimelinePage({
  ...
  return (
    <main className="dark:bg-zinc-900 min-h-screen bg-white dark:bg-zinc-900">
      <div className="sm:px-6 lg:px-8 px-4 py-10 mx-auto max-w-7xl">
        ...
        <div className="relative mt-10">
          <div
            className="bg-slate-200 dark:bg-slate-700 absolute top-0 bottom-0 left-1/2 -ml-px w-px"
            aria-hidden
          />
              ...
              return (
                <li key={year} className="group relative" style={{ marginTop }}>
                  {showYear && (
                    ...
                    >
                      <div className="text-slate-700 dark:text-slate-300 text-sm font-semibold tabular-nums">
                        {year}
                      </div>
                    </div>
                  )}
                  ...
                  <div
                    className={`flex justify-between items-start mt-4 ${lang === "he" ? "flex-row-reverse" : ""}`}
                  >
                    {/* Songs on the left (or right for RTL) */}
                    <div
                      className={`flex-1 mr-4 ${lang === "he" ? "mr-0 ml-4" : ""}`}
                    >
                      {songs.length > 0 && (
                        <SongStack
                    ...
                    <div
                      className={`flex-1 ml-4 relative group ${lang === "he" ? "ml-0 mr-4" : ""}`}
                    >
                      {conflicts.map((conflictEntry) => {
